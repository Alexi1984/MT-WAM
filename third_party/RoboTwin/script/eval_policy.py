...
def _result_suffix_from_task_config(task_config):
    ...
    try:
        return suffix_by_task_config[task_config]
    except KeyError as exc:
        raise ValueError(
            f"Unsupported `task_config` for fixed result naming: {task_config}. Expected one of: {list(suffix_by_task_config)}."
        ) from exc
...
def main(usr_args):
    ...
    if eval_output_dir is not None and str(eval_output_dir).strip() != "":
        ...
        save_dir = Path(
            f"eval_result/{task_name}/{policy_name}/{task_config}/{ckpt_setting}/{eval_ts}"
        )
    ...
    print("============= Config =============\n")
    print(
        "\x1b[95mMessy Table:\x1b[0m "
        + str(args["domain_randomization"]["cluttered_table"])
    )
    print(
        "\x1b[95mRandom Background:\x1b[0m "
    ...
    print(
        "\x1b[95mRandom Table Height:\x1b[0m "
        + str(args["domain_randomization"]["random_table_height"])
    )
    print(
        "\x1b[95mRandom Head Camera Distance:\x1b[0m "
        + str(args["domain_randomization"]["random_head_camera_dis"])
    )
    print(
        "\x1b[94mHead Camera Config:\x1b[0m "
        + str(args["camera"]["head_camera_type"])
        + f", "
        + str(args["camera"]["collect_head_camera"])
    )
    print(
        "\x1b[94mWrist Camera Config:\x1b[0m "
        + str(args["camera"]["wrist_camera_type"])
        + f", "
        + str(args["camera"]["collect_wrist_camera"])
    )
    print("\x1b[94mEmbodiment Config:\x1b[0m " + embodiment_name)
    print("\n==================================")
...
def eval_policy(
    ...
    print(f"\x1b[34mTask Name: {args['task_name']}\x1b[0m")
    print(f"\x1b[34mPolicy Name: {args['policy_name']}\x1b[0m")
    expert_check = True
    TASK_ENV.suc = 0
    TASK_ENV.test_num = 0
    now_id = 0
    ...
    while succ_seed < test_num:
        ...
        if TASK_ENV.eval_video_path is not None:
            ...
            current_video_path = (
                Path(TASK_ENV.eval_video_path) / f"episode{episode_idx}.mp4"
            )
        ...
        if TASK_ENV.eval_video_path is not None:
            ...
            if current_video_path is None or not current_video_path.exists():
                raise FileNotFoundError(
                    f"Expected eval video file not found: {current_video_path}"
                )
        ...
        TASK_ENV.test_num += 1
        print(
            f"\x1b[93m{task_name}\x1b[0m | \x1b[94m{args['policy_name']}\x1b[0m | \x1b[92m{args['task_config']}\x1b[0m | \x1b[91m{args['ckpt_setting']}\x1b[0m\nSuccess rate: \x1b[96m{TASK_ENV.suc}/{TASK_ENV.test_num}\x1b[0m => \x1b[95m{round(TASK_ENV.suc / TASK_ENV.test_num * 100, 1)}%\x1b[0m, current seed: \x1b[90m{now_seed}\x1b[0m\n"
        )
        now_seed += 1
