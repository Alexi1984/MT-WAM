...
class put_bottles_dustbin(Base_Task):
            ...
            while tag and i < gen_lim:
                ...
                for pose in pose_lst:
                    if (
                        np.sum(
                            np.power(
                                np.array(pose[:2]) - np.array(bottle_pose.p[:2]), 2
                            )
                        )
                        < 0.0169
                    ):
    ...
    def play_once(self):
        bottle_lst = sorted(
            self.bottles, key=lambda x: [x.get_pose().p[0] > 0, x.get_pose().p[1]]
        )
        for i in range(self.bottle_num):
            bottle = bottle_lst[i]
            ...
            left_end_action = Action(
                "left", "move", [-0.35, -0.1, 0.93, 0.65, -0.25, 0.25, 0.65]
            )
            if arm_tag == "left":
                ...
                right_action = self.grasp_actor(
                    bottle, arm_tag=arm_tag, pre_grasp_dis=0.1
                )
                ...
                left_action = self.grasp_actor(
                    bottle, arm_tag="left", pre_grasp_dis=0.1
                )
                ...
                self.move(
                    (ArmTag("left"), [left_end_action]), self.back_to_origin("right")
                )
    ...
    def stage_reward(self):
        ...
        for i in range(self.bottle_num):
            ...
            if (
                np.all(np.abs(bottle_pose[:2] - taget_pose) < eps)
                and bottle_pose[2] > 0.2
                and (bottle_pose[2] < 0.7)
            ):
                reward += reward_step
        return reward
