...
class Camera:
    def __init__(self, bias=0, random_head_camera_dis=0, **kwags):
        ...
        self.static_camera_info_list = kwags["left_embodiment_config"][
            "static_camera_list"
        ]
    ...
    def load_camera(self, scene):
        ...
        def create_camera(camera_info, random_head_camera_dis=0):
            ...
            cam_pos = cam_pos + random_dir * np.random.uniform(
                low=0, high=random_head_camera_dis
            )
            cam_forward = np.array(camera_info["forward"]) / np.linalg.norm(
                np.array(camera_info["forward"])
            )
            cam_left = np.array(camera_info["left"]) / np.linalg.norm(
                np.array(camera_info["left"])
            )
        ...
        for i, camera_info in enumerate(self.static_camera_info_list):
            if camera_info.get("forward") == None:
                camera_info["forward"] = (
                    -1 * np.array(camera_info["position"])
                ).tolist()
            ...
            if camera_info["name"] == "head_camera":
                if self.collect_head_camera:
                    ...
                    camera, camera_config = create_camera(
                        camera_info, random_head_camera_dis=self.random_head_camera_dis
                    )
        ...
        observer_mat44[:3, :3] = np.stack(
            [observer_cam_forward, observer_cam_left, observer_up], axis=1
        )
        ...
        world_cam_mat44[:3, :3] = np.stack(
            [world_cam_forward, world_cam_left, world_cam_up], axis=1
        )
    ...
    def get_segmentation(self, level="mesh") -> dict:
        ...
        if self.collect_wrist_camera:
            ...
            res["left_camera"][f"{level}_segmentation"] = _get_segmentation(
                self.left_camera, level=level
            )
            res["right_camera"][f"{level}_segmentation"] = _get_segmentation(
                self.right_camera, level=level
            )
        for camera, camera_name in zip(
            self.static_camera_list, self.static_camera_name
        ):
        ...
        for camera, camera_name in zip(
            ...
            if camera_name == "head_camera":
                if self.collect_head_camera:
                    ...
                    res[camera_name]["depth"] *= (
                        rgba[camera_name]["rgba"][:, :, 3] / 255
                    )
            else:
                res[camera_name] = {}
                res[camera_name]["depth"] = _get_depth(camera)
    ...
    def get_world_pcd(self):
        ...
        def _get_camera_pcd(camera, color=True):
            ...
            res_pcd = (
                np.hstack((points_world_np, points_color_np))
                if color
                else points_world_np
            )
            return res_pcd
    ...
    def get_pcd(self, if_combine=False):
        def _get_camera_pcd(camera):
            ...
            points_world = points_world.squeeze(0)
            if self.pcd_crop:
                min_bound = torch.tensor(self.pcd_crop_bbox[0], dtype=torch.float32).to(
                    device
                )
                max_bound = torch.tensor(self.pcd_crop_bbox[1], dtype=torch.float32).to(
                    device
                )
                inside_bounds_mask = (points_world.squeeze(0) >= min_bound).all(
                    dim=1
                ) & (points_world.squeeze(0) <= max_bound).all(dim=1)
        ...
        if if_combine:
            ...
            for camera, camera_name in zip(
                ...
                if camera_name == "head_camera":
                    if self.collect_head_camera:
                        combined_pcd = np.vstack(
                            (combined_pcd, _get_camera_pcd(camera))
                        )
                else:
