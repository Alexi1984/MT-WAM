...
class place_object_stand(Base_Task):
    ...
    def load_actors(self):
        ...
        if not available_model_ids:
            raise ValueError(
                f"No available model_data.json files found for {self.selected_modelname}"
            )
        ...
        while (object_pos.p[0] - target_rand_pos.p[0]) ** 2 + (
            object_pos.p[1] - target_rand_pos.p[1]
        ) ** 2 < 0.01:
            target_rand_pos = rand_pose(
                xlim=xlim,
                ylim=[-0.15, -0.1],
    ...
    def check_success(self):
        ...
        return (
            np.all(
                abs(object_pose[:2] - displaystand_pose[:2]) < np.array([eps1, eps1])
            )
