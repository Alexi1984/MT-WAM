...
class place_cans_plasticbox(Base_Task):
    ...
    def check_success(self):
        ...
        dis1 = min(
            np.linalg.norm(
                self.object1.get_pose().p[0:2] - plasticbox_functional_points_0
            ),
            np.linalg.norm(
                self.object1.get_pose().p[0:2] - plasticbox_functional_points_1
            ),
        )
        dis2 = min(
            np.linalg.norm(
                self.object2.get_pose().p[0:2] - plasticbox_functional_points_0
            ),
            np.linalg.norm(
                self.object2.get_pose().p[0:2] - plasticbox_functional_points_1
            ),
        )
        threshold = 0.04
        return (
