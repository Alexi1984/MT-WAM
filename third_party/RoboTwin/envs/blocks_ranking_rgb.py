...
class blocks_ranking_rgb(Base_Task):
            ...
            for i in range(3):
                ...
                def check_block_pose(block_pose):
                    for j in range(len(block_pose_lst)):
                        if (
                            np.sum(pow(block_pose.p[:2] - block_pose_lst[j].p[:2], 2))
                            < 0.01
                        ):
                            return False
                    return True
                ...
                while (
                    ...
                    or np.sum(pow(block_pose.p[:2] - np.array([0, -0.1]), 2)) < 0.01
    ...
    def pick_and_place_block(self, block, target_pose=None):
        ...
        if self.last_gripper is not None and self.last_gripper != arm_tag:
            self.move(
                self.grasp_actor(
                    block, arm_tag=arm_tag, pre_grasp_dis=0.09, grasp_dis=0.01
                ),
