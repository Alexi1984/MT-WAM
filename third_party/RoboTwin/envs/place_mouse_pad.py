...
class place_mouse_pad(Base_Task):
    ...
    def load_actors(self):
        ...
        target_rand_pose = rand_pose(
            xlim=xlim, ylim=[-0.2, 0.0], qpos=[1, 0, 0, 0], rotate_rand=False
        )
        while (
            np.sqrt(
    ...
    def check_success(self):
        ...
        eps2 = 0.012
        return (
            np.all(abs(mouse_pose[:2] - target_pos[:2]) < np.array([eps1, eps2]))
            and (
                np.abs(mouse_qpose[2] * mouse_qpose[3] - 0.49) < eps1
