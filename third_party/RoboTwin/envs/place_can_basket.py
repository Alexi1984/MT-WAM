...
class place_can_basket(Base_Task):
    ...
    def load_actors(self):
        ...
        self.can_id = can_dict[self.can_name][
            np.random.randint(0, len(can_dict[self.can_name]))
        ]
    ...
    def play_once(self):
        ...
        if np.linalg.norm(f0[:2] - place_pose[:2]) < np.linalg.norm(
            f1[:2] - place_pose[:2]
        ):
            place_pose = f0
            place_pose[:2] = f0[:2]
            place_pose[3:] = (
                (-1, 0, 0, 0) if self.arm_tag == "left" else (0.05, 0, 0, 0.99)
            )
        else:
            place_pose = f1
            place_pose[:2] = f1[:2]
