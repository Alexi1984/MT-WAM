...
class move_playingcard_away(Base_Task):
    ...
    def play_once(self):
        ...
        self.move(
            self.grasp_actor(
                self.playingcards, arm_tag=arm_tag, pre_grasp_dis=0.1, grasp_dis=0.01
            )
        )
        self.move(
            self.move_by_displacement(arm_tag, x=0.3 if arm_tag == "right" else -0.3)
        )
    ...
    def check_success(self):
        ...
        edge_x = 0.23
        return (
            np.all(abs(playingcards_pose[0]) > abs(edge_x))
