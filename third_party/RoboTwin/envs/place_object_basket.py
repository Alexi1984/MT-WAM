...
class place_object_basket(Base_Task):
    ...
    def load_actors(self):
        ...
        toycar_dict = {"081_playingcards": [0, 1, 2], "057_toycar": [0, 1, 2, 3, 4, 5]}
        self.object_name = ["081_playingcards", "057_toycar"][np.random.randint(0, 2)]
        self.object_id = toycar_dict[self.object_name][
            np.random.randint(0, len(toycar_dict[self.object_name]))
        ]
    ...
    def play_once(self):
        ...
        place_pose = (
            f0
            if np.linalg.norm(f0[:2] - self.object.get_pose().p[:2])
            < np.linalg.norm(f1[:2] - self.object.get_pose().p[:2])
            else f1
        )
        ...
        if not self.plan_success:
            ...
            self.move(
                ...
                self.grasp_actor(
                    self.basket, arm_tag=self.arm_tag.opposite, pre_grasp_dis=0.02
                ),
            )
        else:
            self.move(self.open_gripper(arm_tag=self.arm_tag))
            ...
            self.move(
                self.back_to_origin(arm_tag=self.arm_tag),
                self.grasp_actor(
                    self.basket, arm_tag=self.arm_tag.opposite, pre_grasp_dis=0.08
                ),
            )
        self.move(
