...
class move_pillbottle_pad(Base_Task):
    ...
    def load_actors(self):
        ...
        target_rand_pose = rand_pose(
            xlim=xlim, ylim=[-0.2, 0.1], qpos=[1, 0, 0, 0], rotate_rand=False
        )
        while (
            np.sqrt(
    ...
    def play_once(self):
        ...
        self.move(
            self.grasp_actor(
                self.pillbottle, arm_tag=arm_tag, pre_grasp_dis=0.06, gripper_pos=0
            )
        )
    ...
    def check_success(self):
        ...
        eps1 = 0.03
        return (
            np.all(abs(pillbottle_pos[:2] - target_pos[:2]) < np.array([eps1, eps1]))
            and np.abs(self.pillbottle.get_pose().p[2] - (0.741 + self.table_z_bias))
            < 0.005
