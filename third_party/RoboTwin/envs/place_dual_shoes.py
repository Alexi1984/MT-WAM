...
class place_dual_shoes(Base_Task):
    ...
    def load_actors(self):
        ...
        self.left_shoe = create_actor(
            self, pose=shoes_pose, modelname="041_shoe", convex=True, model_id=shoe_id
        )
        shoes_pose = rand_pose(
            xlim=[0.2, 0.3],
            ylim=[-0.1, 0.05],
    ...
    def play_once(self):
        ...
        self.move(
            ...
            self.move_by_displacement(
                right_arm_tag, x=0.1, y=-0.05, quat=GRASP_DIRECTION_DIC["top_down"]
            ),
        )
    ...
    def check_success(self):
        ...
        return (
            ...
            and np.all(
                abs(right_shoe_pose_p[:2] - (target_pose_p + [0, 0.04])) < eps[:2]
            )
            and np.all(abs(right_shoe_pose_q - target_pose_q) < eps[-4:])
            and (
                abs(left_shoe_pose_p[2] - (self.shoe_box.get_pose().p[2] + 0.01)) < 0.03
            )
            and (
                abs(right_shoe_pose_p[2] - (self.shoe_box.get_pose().p[2] + 0.01))
                < 0.03
            )
