...
class place_a2b_left(Base_Task):
    ...
    def load_actors(self):
        ...
        if not available_model_ids:
            raise ValueError(
                f"No available model_data.json files found for {self.selected_modelname_B}"
            )
    ...
    def play_once(self):
        ...
        target_pose[0] -= 0.13
        self.move(
            self.place_actor(self.object, arm_tag=arm_tag, target_pose=target_pose)
        )
        self.info["info"] = {
    ...
    def check_success(self):
        ...
        return np.all(
            distance < 0.2
            and distance > 0.08
            and (object_pose[0] < target_pos[0])
