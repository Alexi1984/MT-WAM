...
class place_bread_skillet(Base_Task):
    ...
    def play_once(self):
        ...
        self.move(
            self.grasp_actor(
                self.skillet, arm_tag=arm_tag, pre_grasp_dis=0.07, gripper_pos=0
            ),
            self.grasp_actor(
                self.bread, arm_tag=arm_tag.opposite, pre_grasp_dis=0.07, gripper_pos=0
            ),
        )
        self.move(
    ...
    def check_success(self):
        ...
        return (
            np.all(abs(target_pose[:2] - bread_pose[:2]) < [0.035, 0.035])
            and target_pose[2] > 0.76 + self.table_z_bias
            and (bread_pose[2] > 0.76 + self.table_z_bias)
        )
