...
class place_bread_basket(Base_Task):
        ...
        for i in range(2):
            ...
            while True:
                ...
                while (
                    ...
                    or (rand_pos.p[0] - breadbasket_pose.p[0]) ** 2
                ...
                for j in range(len(self.bread)):
                    ...
                    if (peer_pose.p[0] - rand_pos.p[0]) ** 2 + (
                        peer_pose.p[1] - rand_pos.p[1]
                    ) ** 2 < 0.01:
                        pd = False
                        break
                if pd:
    ...
    def check_success(self):
        ...
        for i in range(len(self.bread)):
            ...
            if (
                np.all(abs(pose[:2] - breadbasket_pose[:2]) < np.array([eps1, eps1]))
