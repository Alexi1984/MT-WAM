...
class stack_bowls_three(Base_Task):
        ...
        for i in range(3):
            ...
            def check_bowl_pose(bowl_pose):
                for j in range(len(bowl_pose_lst)):
                    if (
                        np.sum(pow(bowl_pose.p[:2] - bowl_pose_lst[j].p[:2], 2))
                        < 0.0169
                    ):
                        return False
                return True
    ...
    def check_success(self):
        ...
        bowl1_pose, bowl2_pose, bowl3_pose = sorted(
            [bowl1_pose, bowl2_pose, bowl3_pose], key=lambda x: x[2]
        )
        target_height = [
        ...
        eps2 = 0.04
        return (
            np.all(abs(bowl1_pose[:2] - bowl2_pose[:2]) < eps2)
            and np.all(abs(bowl2_pose[:2] - bowl3_pose[:2]) < eps2)
            and np.all(
                np.array([bowl1_pose[2], bowl2_pose[2], bowl3_pose[2]]) - target_height
                < eps
            )
