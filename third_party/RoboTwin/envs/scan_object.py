...
class scan_object(Base_Task):
    ...
    def play_once(self):
        ...
        object_target_pose = (
            ...
            if object_arm_tag == "right"
            else self.left_object_target_pose
        )
        self.move(
            self.place_actor(
                self.object,
    ...
    def check_success(self):
        ...
        target_vec = t3d.quaternions.quat2mat(scanner_func_pose[-4:]) @ np.array(
            [0, 0, -1]
        )
        ...
        eps = 0.025
        return (
            np.all(np.abs(object_pose1 - scanner_func_pose[:3]) < eps)
            and dis > 0
            and (dis < 0.07)
