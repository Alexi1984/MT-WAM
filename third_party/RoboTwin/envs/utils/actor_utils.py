...
class Actor:
    ...
    def get_point(
        ...
        if ret == "matrix":
            ...
            return sapien.Pose(
                world_matrix[:3, 3], t3d.quaternions.mat2quat(world_matrix[:3, :3])
            )
    ...
    def get_contact_point(
        self, idx: int, ret: Literal["matrix", "list", "pose"] = "list"
    ) -> np.ndarray | list | sapien.Pose:
        return self.get_point("contact", idx, ret)

    def iter_contact_points(
        self, ret: Literal["matrix", "list", "pose"] = "list"
...
class ArticulationActor(Actor):
    ...
    def __init__(self, actor: PhysxArticulation, actor_data: dict, mass=0.01):
        assert isinstance(actor, PhysxArticulation), (
            "ArticulationActor must be a Articulation"
        )
        self.actor = actor
        self.config = actor_data
        ...
        for joint in self.actor.get_joints():
            if force_limit is not None:
                joint.set_drive_properties(
                    damping=damping, stiffness=stiffness, force_limit=force_limit
                )
            else:
