...
class Point:
    ...
    def __init__(
        ...
        self.mat = self.word2base(
            self.pose.to_transformation_matrix()
        ).to_transformation_matrix()
        self.base_pose_mat = self.base.get_pose().to_transformation_matrix()
        builder = scene.create_actor_builder()
        builder.set_physx_body_type("static")
        builder.add_visual_from_file(
            filename="./assets/objects/cube/textured.obj", scale=[size, size, size]
        )
    ...
    def trans_mat(to_mat: np.ndarray, from_mat: np.ndarray, scale: float = 1.0):
        ...
        result = np.where(np.abs(result) < 1e-05, 0, result)
        return result
    ...
    def trans_base(
        ...
        now_pose_mat[:3, :3] = (
            base_trans_mat[:3, :3] @ init_pose_mat[:3, :3] @ base_trans_mat[:3, :3].T
        )
    ...
    def set_pose(self, new_pose: sapien.Pose):
        ...
        self.point.set_pose(self.pose)
        self.mat = self.word2base(
            new_pose.to_transformation_matrix()
        ).to_transformation_matrix()
        print("set", self.name)
    ...
    def update(self, force_output: bool = False, flexible: bool = False):
        ...
        self.point.set_pose(self.pose)
        if not np.allclose(new_mat, self.mat, atol=0.001) or force_output:
            self.mat = new_mat
            self.base_pose_mat = self.base.get_pose().to_transformation_matrix()
...
def rotate_along_axis(
    ...
    p = (rotate_mat @ (target_pose.p - center_pose.p).reshape(3, 1)).reshape(
        3
    ) + center_pose.p
...
def add_robot_visual_box(task, pose: sapien.Pose | list, name: str = "box"):
    ...
    builder = scene.create_actor_builder()
    builder.set_physx_body_type("static")
    builder.add_visual_from_file(filename=str(box_path), scale=[0.04] * 3)
...
def get_place_pose(
    ...
    if z_transform:
        z_align_matrix = get_align_matrix(
            actor_pose_mat[:3, :3] @ actor2world[:3, 2], target_z
        )
    else:
    ...
    if constrain == "align":
        ...
        actor_axis_xOy = get_product_vector(target_x, actor_axis) + get_product_vector(
            target_y, actor_axis
        )
        align_axis_xOy = get_product_vector(target_x, align_axis) + get_product_vector(
            target_y, align_axis
        )
        align_mat_xOy = get_align_matrix(actor_axis_xOy, align_axis_xOy)
        actor_pose_mat[:3, :3] = align_mat_xOy @ actor_pose_mat[:3, :3]
    return (
        actor_pose_mat[:3, 3].tolist()
