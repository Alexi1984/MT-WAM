...
def get_all_cluttered_objects():
    ...
    cluttered_objects_config = json.load(
        open(Path("./assets/objects/objaverse/list.json"), "r", encoding="utf-8")
    )
    ...
    for model_dir in objects_dir.iterdir():
        if not model_dir.is_dir():
            continue
        if re.search("^(\\d+)_(.*)", model_dir.name) is None:
            continue
        ...
        for model_cfg in model_dir.iterdir():
            if model_cfg.is_dir() or model_cfg.suffix != ".json":
                continue
            model_id = re.search("model_data(\\d+)", model_cfg.name)
            if not model_id:
                continue
    ...
    same_obj = json.load(
        open(Path("./assets/objects/same.json"), "r", encoding="utf-8")
    )
...
def rand_pose_cluttered(
    ...
    while True:
        ...
        if is_overlap:
            continue
        distances = np.sqrt(
            (np.array([sub_list[0] for sub_list in size_dict]) - x) ** 2
            + (np.array([sub_list[1] for sub_list in size_dict]) - y) ** 2
        )
        max_distances = np.array(
            [sub_list[3] + new_obj_radius + obj_margin for sub_list in size_dict]
        )
        if y - new_obj_radius < 0:
            if z_max > 0.05:
                continue
        if (
            x - new_obj_radius < -0.6
            or x + new_obj_radius > 0.6
...
def create_cluttered_urdf_obj(
    ...
    object: sapien.Articulation = loader.load_multiple(str(modeldir / "model.urdf"))[1][
        0
    ]
