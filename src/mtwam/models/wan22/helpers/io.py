...
class ModelConfig:
    ...
    def parse_download_source(self):
        ...
        env = os.environ.get("MTWAM_DOWNLOAD_SOURCE")
    ...
    def parse_skip_download(self):
        ...
        env = os.environ.get("MTWAM_SKIP_DOWNLOAD")
        if env is None:
            return False
        return env.lower() == "true"

    def reset_local_model_path(self):
        if os.environ.get("MTWAM_MODEL_BASE_PATH") is not None:
            self.local_model_path = os.environ.get("MTWAM_MODEL_BASE_PATH")
        elif self.local_model_path is None:
            self.local_model_path = "./pretrained_models/"
    ...
    def require_downloading(self):
        ...
        if isinstance(origin_file_pattern, list):
            ...
            if all_exist:
                return False
        elif len(glob.glob(os.path.join(local_root, origin_file_pattern))) > 0:
            return False
    ...
    def download(self):
        ...
        patterns = (
            origin_file_pattern
            if isinstance(origin_file_pattern, list)
            else [origin_file_pattern]
        )
        downloaded_files = [
            name for pattern in patterns for name in glob.glob(pattern, root_dir=root)
        ]
        ...
        if download_source == "modelscope":
            ...
            raise ValueError(
                "`download_source` should be `modelscope` or `huggingface`."
            )
    ...
    def download_if_necessary(self):
        ...
        local = (
            ...
            else Path(os.path.expandvars(str(self.model_id))).expanduser()
        )
        explicit_local = local is not None and (
            local.is_dir()
            or local.is_absolute()
            or str(self.model_id).startswith(("./", "../", "~"))
        )
        if self.path is None and explicit_local:
            if not local.is_dir():
                raise FileNotFoundError(f"Model directory not found: {local}")
            patterns = self.origin_file_pattern
            if patterns in (None, "", "./"):
                self.path = str(local.resolve())
            else:
                patterns = patterns if isinstance(patterns, list) else [patterns]
                matches = []
                for pattern in patterns:
                    found = sorted(glob.glob(str(local / pattern)))
                    if not found:
                        raise FileNotFoundError(
                            f"Model files not found: {local / pattern}"
                        )
        ...
        if self.path is None:
            if self.origin_file_pattern in [None, "", "./"]:
                ...
                for pattern in patterns:
                    found = glob.glob(
                        os.path.join(self.local_model_path, self.model_id, pattern)
                    )
                    if not found:
                        raise FileNotFoundError(
                            f"Model files not found: {self.model_id}/{pattern}"
                        )
                    matches.extend(found)
                matches.sort()
                self.path = matches
        ...
        if not paths or any(
            (path is None or not Path(path).exists() for path in paths)
        ):
            raise FileNotFoundError(f"Model assets not found: {self.path}")
...
def load_state_dict(file_path, torch_dtype=None, device="cpu"):
    if isinstance(file_path, list):
        ...
        for file_path_ in file_path:
            state_dict.update(
                load_state_dict(file_path_, torch_dtype=torch_dtype, device=device)
            )
        return state_dict
        ...
        if isinstance(key, str):
            if isinstance(value, dict):
                keys.append(
                    key
                    + "|"
                    + _convert_keys_dict_to_single_str(value, with_shape=with_shape)
                )
            else:
                if with_shape:
...
def hash_model_file(path, with_shape=True):
    ...
    keys_str = _convert_keys_dict_to_single_str(
        keys_dict, with_shape=with_shape
    ).encode("UTF-8")
