...
# Derived from Wan and DiffSynth-Studio; see NOTICE.
# Modified by MT-WAM contributors for model and package integration.
...
def create_group_causal_attn_mask(
    ...
    assert mode in ["causal", "group_diagonal"], (
        f"Mode {mode} must be 'causal' or 'group_diagonal'"
    )
...
class SelfAttention(nn.Module):
    def __init__(
        self, hidden_dim: int, attn_head_dim: int, num_heads: int, eps: float = 1e-06
    ):
        super().__init__()
    ...
    def forward(self, x, freqs, self_attn_mask: Optional[torch.Tensor] = None):
        ...
        x = flash_attention(
            q=q, k=k, v=v, num_heads=self.num_heads, ctx_mask=self_attn_mask
        )
        return self.o(x)
...
class DiTBlock(nn.Module):
    ...
    def forward(
        ...
        x = self.gate(
            x, gate_msa, self.self_attn(input_x, freqs, self_attn_mask=self_attn_mask)
        )
...
class WanVideoDiT(torch.nn.Module):
    def __init__(
        ...
        assert (
            require_vae_embedding == False and fuse_vae_embedding_in_latents == True
    ...
    def _validate_forward_inputs(
        ...
        if x.ndim != 5:
            raise ValueError(
                f"`latents` must be 5D [B, C, T, H, W], got shape {tuple(x.shape)}"
            )
        ...
        if timestep.ndim != 1:
            raise ValueError(
                f"`timestep` must be 1D [B] or [1], got shape {tuple(timestep.shape)}"
            )
        if self.action_conditioned:
            allow_text_only_single_frame = num_latent_frames == 1 and action is None
            if not allow_text_only_single_frame:
                assert action is not None, (
                    "Action input is required for action-conditioned model."
                )
                if action.ndim != 3:
                    raise ValueError(
        ...
        if timestep.shape[0] == 1 and batch_size > 1:
            assert not self.training, (
                "During training, timestep length must match batch_size."
            )
    ...
    def build_video_to_video_mask(
        ...
        dual_f0_branch_slot: bool = False,
    ) -> torch.Tensor:
        if video_seq_len <= 0:
        ...
        if self.video_attention_mask_mode == "per_frame_causal":
            if video_seq_len % video_tokens_per_frame != 0:
                raise ValueError(
                    f"`video_seq_len` must be divisible by `video_tokens_per_frame` in `per_frame_causal` mode, got {video_seq_len} and {video_tokens_per_frame}"
                )
        ...
        if self.video_attention_mask_mode == "first_frame_causal":
            if num_ref_latents < 1:
                raise ValueError(
                    f"`num_ref_latents` must be >= 1, got {num_ref_latents}"
                )
            video_mask = torch.ones(
                (video_seq_len, video_seq_len), dtype=torch.bool, device=device
            )
            ref_tokens = min(num_ref_latents * video_tokens_per_frame, video_seq_len)
            video_mask[:ref_tokens, ref_tokens:] = False
            if dual_f0_branch_slot:
                if num_ref_latents != 2:
                    raise ValueError(
                        f"`dual_f0_branch_slot` requires num_ref_latents == 2 (slot 0 = branch copy, slot 1 = trunk anchor), got {num_ref_latents}"
                    )
            return video_mask
        raise ValueError(
            f"Unsupported video attention mask mode: {self.video_attention_mask_mode}"
        )
    ...
    def pre_dit(
        ...
        if x.shape[3] % patch_h != 0 or x.shape[4] % patch_w != 0:
            raise ValueError(
                f"Latent spatial shape must be divisible by DiT patch size, got HxW=({x.shape[3]}, {x.shape[4]}), patch=({patch_h}, {patch_w})"
            )
        tokens_per_frame = x.shape[3] // patch_h * (x.shape[4] // patch_w)
        if num_ref_latents < 1 or num_ref_latents > x.shape[2]:
            raise ValueError(
                f"`num_ref_latents` must be in [1, T_lat={x.shape[2]}], got {num_ref_latents}"
            )
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            if not hasattr(self, "patch_size") or len(self.patch_size) < 3:
                raise ValueError(
                    f"Invalid dit.patch_size: {getattr(self, 'patch_size', None)}"
                )
            ...
            token_timesteps[:, :num_ref_latents, :] = 0
            token_timesteps = token_timesteps.reshape(batch_size, -1)
            token_t_emb = sinusoidal_embedding_1d(
                self.freq_dim, token_timesteps.reshape(-1)
            )
            t = self.time_embedding(token_t_emb).reshape(
        ...
        if self.action_conditioned and action is not None:
            ...
            num_temporal_groups = f - num_ref_latents
            if num_temporal_groups <= 0:
                raise ValueError(
                    "Action-conditioned context mask requires at least one predicted latent frame when `action` is provided."
                )
            ...
            final_context_mask[
                :, num_ref_latents * tokens_per_frame :, context_len:
            ] = action_group_mask.unsqueeze(0).expand(batch_size, -1, -1)
            context_mask = final_context_mask
        elif self.action_conditioned and action is None:
            if f != num_ref_latents:
                raise ValueError(
                    f"Action-conditioned model requires `action` unless running reference-only mode (T_lat == num_ref_latents), got T_lat={f} vs num_ref_latents={num_ref_latents}."
                )
    ...
    def post_dit(
        self, x_tokens: torch.Tensor, pre_state: Dict[str, Any]
    ) -> torch.Tensor:
