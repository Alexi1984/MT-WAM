...
# licenses/PyTorch3D-BSD-3-Clause.txt file distributed with this project.
...
class PoseRotationTransform:
    ...
    def _split_pose(
        self, pose: torch.Tensor, rotation_type: str
        ...
        others = (
            pose[..., 3 + rotation_dim :]
            if pose.shape[-1] > 3 + rotation_dim
            else torch.empty_like(position[..., :0])
        )
        assert rotation.shape[-1] == rotation_dim, (
            f"Expected {rotation_dim} dims for {rotation_type}"
        )
    ...
    def _rotation_to_matrix(
        self, rotation: torch.Tensor, rotation_type: str
    ) -> torch.Tensor:
        if rotation_type == "quaternion":
    ...
    def _matrix_to_rotation(
        self, matrix: torch.Tensor, rotation_type: str
    ) -> torch.Tensor:
        if rotation_type == "quaternion":
