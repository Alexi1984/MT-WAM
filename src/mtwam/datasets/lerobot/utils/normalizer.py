...
class LinearNormalizer:
    ...
    def get_stats(self):
        stats = {
            ...
            "state": {
                key: norm.get_stats() for key, norm in self.normalizers["state"].items()
            },
        }
        return stats
...
def search_dataset_stats_cache_json(
    ...
    dataset_hash = hashlib.sha256(
        signature_json.encode("utf-8"), usedforsecurity=False
    ).hexdigest()
