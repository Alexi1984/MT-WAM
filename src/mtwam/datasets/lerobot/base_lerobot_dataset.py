...
from typing import List, Literal, Dict, Optional, Any, DefaultDict, Union
...
def filter_excluded_episodes(
    episodes: Dict[str, List[int]],
    exclude_episodes: Optional[Union[List[int], Dict[str, List[int]]]],
) -> Dict[str, List[int]]:
    if not exclude_episodes:
        return episodes
    if hasattr(exclude_episodes, "keys"):
        unknown = [k for k in exclude_episodes.keys() if k not in episodes]
        if unknown:
            raise ValueError(
                f"[exclude_episodes] keys {sorted(unknown)} match no loaded repo_id (loaded: {sorted(episodes)}). Use the exact dataset_dirs path as the key."
            )
        per_repo = {
            repo_id: {int(e) for e in exclude_episodes.get(repo_id) or []}
            for repo_id in episodes
        }
    else:
        all_excluded = {int(e) for e in exclude_episodes}
        per_repo = {repo_id: all_excluded for repo_id in episodes}
    out: Dict[str, List[int]] = {}
    for repo_id, ep_list in episodes.items():
        excluded = per_repo[repo_id]
        kept = [e for e in ep_list if e not in excluded]
        removed = len(ep_list) - len(kept)
        if removed:
            logger.info(
                "[exclude_episodes] %s: excluded %s -> removed %d episode(s), %d remain",
                repo_id,
                sorted(excluded),
                removed,
                len(kept),
...
def filter_included_episodes(
    episodes: Dict[str, List[int]],
    include_episodes: Optional[Union[List[int], Dict[str, List[int]]]],
) -> Dict[str, List[int]]:
    if not include_episodes:
        return episodes
    if hasattr(include_episodes, "keys"):
        unknown = [k for k in include_episodes.keys() if k not in episodes]
        if unknown:
            raise ValueError(
                f"[include_episodes] keys {sorted(unknown)} match no loaded repo_id (loaded: {sorted(episodes)}). Use the exact dataset_dirs path as the key."
            )
        per_repo = {
            repo_id: {int(e) for e in include_episodes[repo_id]}
            if repo_id in include_episodes
            else None
            for repo_id in episodes
        }
    else:
        all_included = {int(e) for e in include_episodes}
        per_repo = {repo_id: all_included for repo_id in episodes}
    out: Dict[str, List[int]] = {}
    for repo_id, ep_list in episodes.items():
        included = per_repo[repo_id]
        if included is None:
            out[repo_id] = ep_list
            continue
        kept = [e for e in ep_list if e in included]
        logger.info(
            "[include_episodes] %s: whitelist %d -> kept %d / %d episode(s)",
            repo_id,
            len(included),
            len(kept),
            len(ep_list),
...
def load_episode_ids_file(path: str) -> List[int]:
    out: List[int] = []
    with open(path) as f:
        for line in f:
            s = line.strip()
...
class BaseLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        exclude_episodes: Optional[Union[List[int], Dict[str, List[int]]]] = None,
        include_episodes: Optional[Union[List[int], Dict[str, List[int]]]] = None,
    ):
        assert len(dataset_dirs) > 0, "At least one dataset directory is required"
        assert past_action_size >= 0 and past_obs_size >= 0
        assert past_action_size == past_obs_size, (
            f"obs and action windows must shift together (same left extension), got past_action_size={past_action_size} vs past_obs_size={past_obs_size}"
        )
        ...
        episodes = filter_excluded_episodes(episodes, exclude_episodes)
        episodes = filter_included_episodes(episodes, include_episodes)
    ...
    def __getitem__(self, idx):
        ...
        while attempt < MAX_GETITEM_ATTEMPT:
            try:
                ...
                from .teacher_cache import derive_teacher_window_id_from_raw

                window_ids = derive_teacher_window_id_from_raw(self, lerobot_sample)
                lerobot_sample = self._split_lerobot_sample(lerobot_sample)
                break
