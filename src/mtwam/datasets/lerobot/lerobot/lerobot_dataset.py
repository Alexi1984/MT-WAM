...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def add_frame(
        self, frame: dict, task: List[str], timestamp: float | None = None
    ) -> None:
...
class MultiLeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.tolerances_s = (
            tolerances_s if tolerances_s else dict.fromkeys(ds_names, 0.0001)
        )
        self._datasets = []
        for ds_root, ds_name in zip(ds_roots, ds_names, strict=True):
            try:
                ...
                logging.error(
                    f"Exception while process ds_root: {ds_root}, ds_name: {ds_name}"
                )
                traceback.print_exc()
                raise e
    ...
    def get_episode_data(self, episode_idx: int) -> dict:
        for dataset in self._datasets:
            if episode_idx < dataset.num_episodes:
                file = str(
                    dataset.root
                    / dataset.meta.get_data_file_path(dataset.episodes[episode_idx])
                )
