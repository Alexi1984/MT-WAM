...
from mtwam.utils.pytorch_utils import dict_apply
...
class MTWAMProcessor(BaseProcessor):
    def __init__(
        self,
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.num_output_cameras > pixel_values.shape[0]:
            logger.warning(
                f"num_output_cameras {self.num_output_cameras} exceeds the number of cameras in data {pixel_values.shape[0]}, ZERO-PADDING the missing cameras — check shape_meta.images vs num_output_cameras."
            )
            out = torch.zeros(
            ...
            logger.warning(
                f"num_output_cameras {self.num_output_cameras} is less than the number of cameras in data {pixel_values.shape[0]}, truncating the input to the first {self.num_output_cameras} cameras."
            )
