...
from mtwam.models.wan22.helpers.loader import _load_registered_model, _resolve_configs
from mtwam.models.wan22.wan_video_text_encoder import HuggingfaceTokenizer
from mtwam.utils.config_resolvers import register_default_resolvers
from mtwam.utils.logging_config import get_logger, setup_logging
...
def _node_include_episodes(node: DictConfig, node_path: str):
    inc = node.get("include_episodes")
    inc_file = node.get("include_episodes_file")
    if not inc and inc_file:
        from mtwam.datasets.lerobot.base_lerobot_dataset import load_episode_ids_file

        inc = load_episode_ids_file(str(inc_file))
    if not inc:
        return None
    if isinstance(inc, (DictConfig, dict)):
        out = set()
        for ids in inc.values():
            out.update((int(e) for e in ids))
    else:
        out = {int(e) for e in inc}
    logger.info("Dataset node `%s` whitelists %d episode(s).", node_path, len(out))
    return out
...
def _collect_dataset_settings(data_cfg: DictConfig):
    ...
    for node_path, node in _iter_dataset_nodes(data_cfg, path="data"):
        ...
        if cache_dir is None or not str(cache_dir).strip():
            raise ValueError(
                f"Missing `text_embedding_cache_dir` for dataset node `{node_path}` (this node defines `dataset_dirs`)."
            )
        for ds in raw_dirs:
            ds_str = str(ds)
        ...
        saw_node = True
        node_inc = _node_include_episodes(node, node_path)
        if node_inc is None:
            every_node_whitelisted = False
        else:
            include_episodes.update(node_inc)
        logger.info(
    ...
    subset = (
        include_episodes
        if saw_node and every_node_whitelisted and include_episodes
        else None
    )
    return (dataset_dirs, cache_dirs, context_lens, subset)


def _resolve_context_len(context_lens: set[int]) -> int:
    if len(context_lens) != 1:
        raise ValueError(
            f"Found multiple context_len values in data config: {sorted(context_lens)}. Please keep them consistent."
        )
...
def _model_id_to_enc_id(model_id: str) -> str:
    ...
    enc_id = re.sub("[^a-z0-9]+", "", base.lower())
...
def main(cfg: DictConfig):
    ...
    dataset_dirs, cache_dirs, context_lens, subset_episodes = _collect_dataset_settings(
        cfg.data
    )
    if not cache_dirs:
    ...
    if override_prompt is not None:
        ...
        if subset_episodes is None:
            prompts = _read_unique_prompts(dataset_dirs)
        else:
            prompts = collect_prompts_for_episodes(dataset_dirs, subset_episodes)
            logger.info(
                "Episode-whitelist mode: %d episode(s) -> %d unique prompt(s).",
                len(subset_episodes),
                len(prompts),
            )
    if not prompts:
        ...
        return
    if _to_bool(cfg.get("text_embed_dry_run", False)):
        logger.info(
            "DRY RUN: would encode %d prompt(s) into %s (context_len=%d). Nothing written.",
            len(prompts),
    ...
    if is_distributed:
        ...
        count_tensor = torch.tensor(
            [prompts_encoded_local], device=reduce_device, dtype=torch.long
        )
        dist.all_reduce(count_tensor, op=dist.ReduceOp.SUM)
        ...
        with torch.no_grad():
            for start in range(0, len(prompts), DEFAULT_BATCH_SIZE):
                ...
                for i, prompt in enumerate(batch_prompts):
                    ...
                    mask_i = (
                        mask[i].detach().to(device="cpu", dtype=torch.bool).contiguous()
                    )
                    payload = {"context": context_i, "mask": mask_i}
                    for cache_dir in cache_dirs:
                        cache_path = (
                            cache_dir / f"{hashed}.t5_len{context_len}.{enc_id}.pt"
                        )
                        key = str(cache_dir)
    ...
    if is_distributed:
        ...
        over_tensor = torch.tensor(
            [over_length_prompts], device=reduce_device, dtype=torch.long
        )
