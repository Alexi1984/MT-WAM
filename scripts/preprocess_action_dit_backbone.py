...
from mtwam.models.wan22.helpers.loader import load_wan22_ti2v_5b_components
...
def _load_model_config(config_name, overrides=()):
    from hydra import compose, initialize_config_dir

    if config_name not in ("train_libero", "train_robotwin"):
        raise ValueError(f"Unsupported training configuration: {config_name}")
    config_dir = Path(__file__).resolve().parents[1] / "configs"
    with initialize_config_dir(config_dir=str(config_dir), version_base="1.3"):
        root = compose(config_name=config_name, overrides=list(overrides))
    OmegaConf.resolve(root)
    model = root.model
    return (
        OmegaConf.to_container(model.video_dit_config, resolve=True),
        OmegaConf.to_container(model.action_dit_config, resolve=True),
        model,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config-name",
        choices=["train_libero", "train_robotwin"],
        default="train_libero",
    )
    parser.add_argument(
        "--output", required=True, help="Output path for ActionDiT backbone weights."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--video-dit-path",
        default=None,
        help="Optional explicit path or glob for Wan video DiT weights.",
    )
    parser.add_argument(
        "overrides",
        nargs="*",
        help="Hydra configuration overrides, such as paths.wan=./pretrained_models/Wan2.2-TI2V-5B",
    )
    args = parser.parse_args()
    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    apply_alpha_scaling = _parse_bool(args.apply_alpha_scaling)
    video_cfg, action_cfg, cfg = _load_model_config(args.config_name, args.overrides)
    ...
    print(
        f"[INFO] Loaded model config from {args.config_name}. Preprocessing ActionDiT backbone with dtype={torch_dtype} on device={args.device}, apply_alpha_scaling={apply_alpha_scaling}."
    )
    components = load_wan22_ti2v_5b_components(
        ...
        video_dit_pretrained_path=args.video_dit_path,
    )
    if args.video_dit_path:
        print(
            f"[INFO] Video DiT source override: {components.dit_path} (backbone lineage follows it)."
        )
    ...
    print(
        f"[INFO] Saved ActionDiT backbone payload to {output_path} (copied={copied}, interpolated={interpolated}, skipped={skipped})."
    )
