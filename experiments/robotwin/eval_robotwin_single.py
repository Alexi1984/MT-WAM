...
def _task_config_label(task_config: Any) -> str:
    task_config_text = str(task_config)
    try:
        return RESULT_LABEL_BY_TASK_CONFIG[task_config_text]
    except KeyError as exc:
        raise ValueError(f"Unsupported C2R task config: {task_config_text}") from exc
...
def _ensure_policy_symlink(robotwin_root: Path, policy_source_dir: Path) -> Path:
    ...
    if policy_target.is_symlink():
        ...
        if target_resolved != source_resolved:
            raise RuntimeError(
                f"Policy symlink conflict: {policy_target} -> {target_resolved}, expected -> {source_resolved}"
            )
        return policy_target
    raise RuntimeError(
        f"Path already exists and is not a symlink: {policy_target}. Please handle it manually to avoid overriding existing policy files."
    )
...
def main(cfg: DictConfig):
    ...
    if not EVAL_POLICY_ENTRY.is_file():
        raise FileNotFoundError(
            f"MTWAM RoboTwin eval entry not found: {EVAL_POLICY_ENTRY}"
        )
    ...
    if not policy_source_dir.is_dir():
        raise FileNotFoundError(
            f"Policy source directory not found: {policy_source_dir}"
        )
    _ensure_policy_symlink(
        robotwin_root=robotwin_root, policy_source_dir=policy_source_dir
    )
    run_output_dir = _resolve_path(str(cfg.EVALUATION.output_dir), base=PROJECT_ROOT)
    run_output_dir.mkdir(parents=True, exist_ok=True)
    config_label = _task_config_label(cfg.EVALUATION.task_config)
    log_file = (
        run_output_dir
        / f"eval_{str(cfg.EVALUATION.task_name)}_{config_label}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
    robotwin_eval_base = run_output_dir / str(cfg.EVALUATION.task_name)
    sim_cfg_path = save_resolved_config(
        cfg, robotwin_eval_base / f"{config_label}_policy.yaml", PROJECT_ROOT
    )
    ...
    gpu_ids = visible.split(",") if visible else None
    if gpu_ids is not None and (not 0 <= int(cfg.gpu_id) < len(gpu_ids)):
        raise ValueError("gpu_id exceeds CUDA_VISIBLE_DEVICES.")
    env["CUDA_VISIBLE_DEVICES"] = (
        gpu_ids[int(cfg.gpu_id)] if gpu_ids is not None else str(cfg.gpu_id)
    )
    ...
    OmegaConf.save(
        ...
        f=str(
            run_output_dir
            / f"eval_config_{str(cfg.EVALUATION.task_name)}_{config_label}.yaml"
        ),
    )
